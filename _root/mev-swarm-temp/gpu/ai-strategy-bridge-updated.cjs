#!/usr/bin/env node
// ai-strategy-bridge-updated.cjs — CommonJS wrapper with dead-man safety check
...
console.log('Bridge (updated .cjs) watching for', advicePath);
...
async function tryPublish(report) {
  // Require deadman to be armed before any external publish
  if (!deadman.isArmed()) {
    console.warn('DEADMAN not armed: aborting external publish. To arm, set DEADMAN_ARMED=true or create DEADMAN_ARMED file at repo root.');
    // still write local signal file for inspection
    writeSignal(report);
    return;
  }
