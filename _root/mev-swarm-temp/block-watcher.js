...
// Debug logging override - bypass dotenv quiet mode when enabled
...
if (process.platform === 'win32') {
  try {
    ...
    process.title = 'WATCHER - Block Watcher (Polling Mempool)';
    console.log('🏷️  Process Label: WATCHER');
  } catch (e) {
...
// ============================================================
// 🔧 UTILITY MODULES - Caching, RPC, Token Discovery
// ============================================================
import { rpcCall } from './utils/rpc.js';
import { getToken, setToken } from './utils/cache.js';
import { resolveToken } from './utils/tokens.js';

// Expanded token list for display purposes
const COMMON_TOKENS = {
...
// Token decimals map (comprehensive)
const TOKEN_DECIMALS = {
  ...
  'REDACTED_ADDRESS': 18, // PEPE
  'REDACTED_ADDRESS': 6,  // USDT
};

// Token metadata using utils cache
async function getTokenMeta(address, provider) {
  if (!address) return { symbol: 'UNKNOWN', decimals: 18 };

  const key = address.toLowerCase();

  // Use utils cache first
  const cached = getToken(key);
  if (cached) return cached;

  // Try COMMON_TOKENS first (for display)
  const symbol = COMMON_TOKENS[key];
  const decimals = TOKEN_DECIMALS[key];

  if (symbol) {
    const meta = { symbol, decimals, address };
    setToken(key, meta);
    return meta;
  }
  ...
  try {
    const meta = await resolveToken(address, provider);
    return meta;
  } catch (err) {
...
// ============================================================
// ETH Price Cache (fetches from Chainlink price feed)
// ============================================================
let cachedEthPrice = null;
let ethPriceTimestamp = 0;
const ETH_PRICE_CACHE_TTL = 60000; // 1 minute

async function getEthPriceUsd(provider) {
  const now = Date.now();
  if (cachedEthPrice && (now - ethPriceTimestamp < ETH_PRICE_CACHE_TTL)) {
    return cachedEthPrice;
  }

  try {
    // Chainlink ETH/USD price feed on mainnet
    const priceFeedAddress = 'REDACTED_ADDRESS';
    ...
    cachedEthPrice = Number(data.price) / 1e8;
    ethPriceTimestamp = now;
    console.log(`   💰 Fetched ETH price: $${cachedEthPrice.toFixed(2)}`);
    return cachedEthPrice;
  } catch (error) {
    console.log(`   ⚠️  Failed to fetch ETH price, using fallback: $3000`);
    return 3000; // Fallback if oracle unavailable
  }
}
...
function decodeV3Path(pathHex) {
  ...
  while (offset + 40 <= path.length) {
    const tokenIn = "0x" + path.slice(offset, offset + 40);
    offset += 40;

    if (offset + 6 > path.length) break;
    const fee = parseInt(path.slice(offset, offset + 6), 16);
    offset += 6;

    if (offset + 40 > path.length) break;
    const tokenOut = "0x" + path.slice(offset, offset + 40);
    offset += 40;

    hops.push({ tokenIn: tokenIn.toLowerCase(), tokenOut: tokenOut.toLowerCase(), fee });
  }
...
// ============================================================
// Handle Uniswap V3 swaps with token metadata
// ============================================================
async function handleUniswapV3Swap(tx, decoded, provider) {
  // exactInputSingle: single hop
  if (decoded.name === "exactInputSingle") {
    console.log(`   🐛 V3 args:`, decoded.args);

    const { tokenIn, tokenOut, amountIn, amountIn: amountInRaw } = decoded.args;

    if (!tokenIn || !tokenOut) {
      console.log(`   ⚠️  Missing token addresses in V3 swap`);
      return;
    }

    const [inMeta, outMeta] = await Promise.all([
      getTokenMeta(tokenIn, provider),
      getTokenMeta(tokenOut, provider),
    ]);

    const humanAmountIn = amountInRaw && amountInRaw !== BigInt(0) ? Number(ethers.formatUnits(amountInRaw, inMeta.decimals)) : 0;

    console.log(
      `🔄 DEX SWAP [Uniswap V3] | ${inMeta.symbol} → ${outMeta.symbol} (${humanAmountIn.toFixed(4)} ${inMeta.symbol}) | exactInputSingle`
    );
    return;
  }

  // exactInput: multi-hop packed path
  if (decoded.name === "exactInput") {
    const { path, amountIn } = decoded.args;
    const hops = decodeV3Path(path);

    if (!hops.length) {
      console.log("🔄 DEX SWAP [Uniswap V3] | 0x0000... → 0x0000... (0.0000) | exactInput");
      console.log("   ⚠️  Price impact unavailable: Path decode failed");
      return;
    }
    ...
    const [inMeta, outMeta] = await Promise.all([
      getTokenMeta(first.tokenIn, provider),
      getTokenMeta(last.tokenOut, provider),
    ]);

    const humanAmountIn = amountIn ? Number(ethers.formatUnits(amountIn, inMeta.decimals)) : 0;

    const routeSymbols = [inMeta.symbol];
    for (const hop of hops) {
      const meta = await getTokenMeta(hop.tokenOut, provider);
      routeSymbols.push(meta.symbol);
    }

    console.log(
      `🔄 DEX SWAP [Uniswap V3] | ${routeSymbols.join(" → ")} (${humanAmountIn.toFixed(4)} ${inMeta.symbol}) | exactInput`
    );
  }
}

// Uniswap V2 Factory & Pair Addresses (Mainnet)
const UNISWAP_V2_FACTORY = 'REDACTED_ADDRESS';

// Sushiswap Factory (Mainnet)
const SUSHISWAP_FACTORY = 'REDACTED_ADDRESS';
...
class PriceImpactCalculator {
  constructor(provider) {
    ...
    // Use utils/cache.js for pool caching
    this.cacheTimeout = 5000;
    this.localCache = new Map(); // For timestamp-based caching on top of utils cache
  }

  /**
   * Get pair address for a token pair from a factory using utils/cache.js
   */
  async getPairAddress(tokenA, tokenB, dexName) {
    const sorted = [tokenA, tokenB].sort();
    const key = `${dexName}-${sorted[0]}-${sorted[1]}`;

    // Check utils/cache.js first
    const cachedPool = getPool(key);
    if (cachedPool && cachedPool.address) {
      if (Date.now() - cachedPool.timestamp < this.cacheTimeout) {
        return cachedPool.address;
      }
    }
    ...
    try {
      ...
      setPool(key, { address: pairAddress, timestamp: Date.now() });
      this.localCache.set(key, { address: pairAddress, timestamp: Date.now() });
  ...
  /**
   * Get reserves for a pair using eth_call and utils/cache.js
   */
  async getReserves(pairAddress) {
    ...
    // Check local cache for timestamp-based expiry
    ...
    try {
      // Use rpcCall for rate-limited request
      const reserves = await rpcCall(async () => {
      ...
      this.localCache.set(cacheKey, { reserves: result, timestamp: Date.now() });
      return result;
    } catch (e) {
      return null;
    }
...
class BlockSimulator {
  ...
  async simulatePendingTx(tx, amountIn) {
    const dexInfo = await parseDexTransaction(tx, this.provider);
...
function decodeSwapData(input, value, dexName) {
  // Force debug log to verify DEBUG_LOGS is working
  if (DEBUG_LOGS && Math.random() < 0.01) { // Log 1% of calls to verify
    console.log(`🐛 DEBUG_LOGS is working! Called with dexName: ${dexName}`);
  }
  ...
  try {
    ...
    if (funcName === 'swapExactETHForTokens' || funcName === 'swapExactTokensForETH') {
      ...
      if (funcName === 'swapExactETHForTokens') {
        ...
        tokenIn = params[1][0];
        tokenOut = 'REDACTED_ADDRESS'; // WETH
        amountIn = params[0];
        amountOut = value;
      }
      ...
      amountIn = params[0];
      amountOut = params[1];
    } else if (funcName === 'exactInputSingle' || funcName === 'exactInput') {
      // 🆕 Enhanced Uniswap V3 handling with path decoding and token metadata
      if (dexName === 'Uniswap V3') {
        try {
          const iface = new ethers.Interface([
            "function exactInputSingle((address,address,uint24,address,uint256,uint256,uint256,uint160))",
            "function exactInput(bytes path,uint256 amountIn,uint256 amountOutMin,address recipient,uint256 deadline)"
          ]);

          const decoded = iface.parseTransaction({ data: input });

          if (decoded && decoded.name) {
            console.log(`   ✅ Enhanced V3 decode: ${decoded.name}`);
            // Return enhanced data that will be handled by parseDexTransaction
            return {
            ...
            console.log(`   ⚠️  V3 parse returned null or no name`);
          }
        } catch (e) {
          console.log(`   ⚠️  V3 decode failed: ${e.message}`);
        }
      }

      // Fallback to original decoding if not enhanced
    ...
    // Debug: Log function names that don't have decoder
    if (DEBUG_LOGS && !tokenIn && !tokenOut && funcName) {
      console.log(`  🐛 Function '${funcName}' has no decoder implemented`);
      console.log(`  🐛 Input data: ${input.slice(0, 100)}...`);
    }
 ...
 * 🆕 Enhanced to handle V3 path decoding with token metadata
 */
async function parseDexTransaction(tx, provider) {
  const { isDex, dexName } = isDexTransaction(tx.to);
  if (!isDex) return null;

  const swapData = decodeSwapData(tx.data, tx.value, dexName);

  // Debug: Log when we have a DEX tx but no swap data
  if (DEBUG_LOGS && !swapData) {
    console.log(`  🐛 DEX tx detected but no swap data: ${tx.hash.slice(0, 10)}... to ${tx.to}`);
    console.log(`  🐛 Data field: ${tx.data ? tx.data.slice(0, 50) + '...' : 'MISSING'}`);
    const funcSig = tx.data ? tx.data.slice(0, 10) : 'none';
    console.log(`  🐛 Func sig: ${funcSig} (${DEX_FUNCTIONS[funcSig] || 'UNKNOWN'})`);
  }

  // 🆕 Handle enhanced Uniswap V3 swaps
  if (swapData && swapData.isEnhancedV3 && swapData.decoded) {
    await handleUniswapV3Swap(tx, swapData.decoded, provider);
    return { dex: dexName, to: tx.to, hash: tx.hash, ...swapData };
  }

  if (!swapData) {
    return { dex: dexName, to: tx.to, hash: tx.hash, isSwap: true, note: 'Contract interaction (undecoded)' };
  }
  return { dex: dexName, to: tx.to, hash: tx.hash, ...swapData };
}
...
class BlockWatcher {
  constructor(onBlock) {
    ...
    this.lastRpcCall = 0;
    this.minRpcInterval = 100; // 100ms minimum between RPC calls
    this.rpcCallCount = 0;
  }

  /**
   * 🆕 Rate-limited RPC call wrapper
   * Ensures minimum time between RPC calls to prevent overwhelming the endpoint
   */
  async rateLimitedRpcCall(asyncCallFn) {
    const now = Date.now();
    const timeSinceLastCall = now - this.lastRpcCall;

    if (timeSinceLastCall < this.minRpcInterval) {
      const waitTime = this.minRpcInterval - timeSinceLastCall;
    ...
    // Log RPC call count every 100 calls for monitoring
    if (this.rpcCallCount % 100 === 0) {
      console.log(`📡 RPC calls: ${this.rpcCallCount} (rate-limited)`);
    }

    // Use utils/rpc.js for automatic rate limit handling with retry
    return await rpcCall(asyncCallFn);
  }
  ...
  async start() {
    ...
    try {
      ...
      // Start polling (setInterval handles first call - no duplicate pollMempool())
  ...
  async pollMempool() {
    ...
    try {
      // FIRST: Check for new mined blocks (rate-limited)
      const currentBlock = await this.rateLimitedRpcCall(() => this.provider.getBlockNumber());
      ...
      // THEN: Get pending block (transactions in mempool) - rate-limited
      const pendingBlock = await this.rateLimitedRpcCall(() =>
        this.provider.send('eth_getBlockByNumber', ['pending', false])
      );
      
      // Debug: log what we got
      if (!pendingBlock || !pendingBlock.transactions || pendingBlock.transactions.length === 0) {
        // Try latest block instead as fallback (rate-limited)
        const latestBlock = await this.rateLimitedRpcCall(() =>
      ...
      if (pendingBlock && pendingBlock.transactions) {
        // 🔒 LIMIT: Process pending transactions (configurable via MAX_PENDING_TXS env var)
        const MAX_PENDING_TXS = parseInt(process.env.MAX_PENDING_TXS) || 150;
        if (pendingBlock.transactions.length > MAX_PENDING_TXS) {
            console.log(`⚠️  Pending block has ${pendingBlock.transactions.length} transactions, processing first ${MAX_PENDING_TXS}`);
        }
        const txs = pendingBlock.transactions.slice(0, MAX_PENDING_TXS);
        ...
        for (const txHash of txs) {
          ...
          try {
            // Get full transaction details (rate-limited)
            const tx = await this.rateLimitedRpcCall(() =>
...
async function demo() {
  ...
  const watcher = new BlockWatcher(async (blockNumber, data) => {
    if (data.type === 'pending_tx' && data.transaction) {
      ...
      // Sample first few transactions to debug structure
      if (DEBUG_LOGS && watcher.txCount <= 5) {
        console.log(`\n🔍 [TX#${watcher.txCount}] ${tx.hash.slice(0, 10)}...`);
        console.log(`   To: ${tx.to}`);
        console.log(`   Has data: ${!!tx.data && tx.data !== '0x'}`);
        if (tx.data && tx.data !== '0x') {
          console.log(`   Func sig: ${tx.data.slice(0, 10)}`);
        }
      }

      // Try to parse as DEX swap (🆕 with provider for token metadata)
      const dexInfo = await parseDexTransaction(tx, watcher.provider);
      
      if (dexInfo && dexInfo.isSwap) {
        dexSwapCount++;
        const valueEth = (tx.value / BigInt(1e18)).toString();

        // Debug: Log undecoded swaps
        if (DEBUG_LOGS && (!dexInfo.tokenIn || !dexInfo.tokenOut)) {
          console.log(`\n🐛 UNDECODED SWAP [${dexInfo.dex}]`);
          console.log(`   Hash: ${tx.hash}`);
          console.log(`   To: ${tx.to}`);
          console.log(`   Note: ${dexInfo.note || 'No details'}`);
          console.log(`   Data: ${tx.data ? tx.data.slice(0, 100) + '...' : 'MISSING'}`);
          if (tx.data && tx.data !== '0x') {
            ...
            if (!impact.error) {
              ...
              if (parseFloat(dexInfo.amountIn) > 1) {
                ...
                if (comparison.prices && comparison.prices.length > 0) {
                  ...
                  // 🆕 Calculate and log expected profit in dollars FIRST (before creating event)
                  let profitUsd = 0;
                  let tokenInfo = { symbol: 'UNKNOWN', decimals: 18 };

                  if (comparison.recommendation.includes('ARB')) {
                    const best = parseFloat(comparison.prices[0].expectedOutput);
                    const worst = parseFloat(comparison.prices[comparison.prices.length - 1].expectedOutput);
                    const profitPerUnit = best - worst;

                    // Get token info to determine value
                    tokenInfo = await getTokenMeta(dexInfo.tokenInAddr, watcher.provider);

                    // Calculate notional size in USD using live ETH price
                    const amountInEth = Number(dexInfo.amountInRaw) / Math.pow(10, tokenInfo.decimals);
                    const ethPrice = await getEthPriceUsd(watcher.provider);

                    profitUsd = profitPerUnit * ethPrice;

                    console.log(`\n🎯 [WATCHER] Opportunity detected: ${tokenInfo.symbol} → ... → ${tokenInfo.symbol}`);
                    console.log(`   Spread: ${comparison.arbitragePercent}`);
                    console.log(`   Expected profit (${amountInEth.toFixed(4)} ${tokenInfo.symbol}): $${profitUsd.toFixed(2)}\n`);
                  }

                  // 🆕 EMIT OPPORTUNITY EVENT FOR EXECUTOR (AFTER profitUsd and tokenInfo are defined)
                  if (profitUsd > 0) {
                    // Get route symbols with proper await
                    const routeMetas = await Promise.all(
                      dexInfo.route.map(addr => getTokenMeta(addr, watcher.provider))
                    );
                    const routeSymbols = routeMetas.map(meta => meta.symbol);

                    const opportunityEvent = {
                      route: routeSymbols,
                      routeType: '2-hop',
                      dexes: [dexInfo.dex, dexInfo.dex],
                      amountIn: dexInfo.amountInRaw,
                    ...
                    // Emit to any connected listeners (e.g., executor, database)
                    console.log(`\n📡 EMITTING OPPORTUNITY EVENT: ${opportunityEvent.route.join(' → ')}`);
